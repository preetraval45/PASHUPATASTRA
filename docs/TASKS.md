...
- `[!]` **T-3.5.1 — IRSA per agent** · M — blocked by T-0.9
  - a. One role per agent, mirroring its declared permissions
  - b. Read and write roles separated
  - c. **Destructive permissions granted to no role, in any environment** — the platform's promises should be true at the IAM layer, not only in its own code
- `[!]` **T-3.5.2 — Secrets** · S — Secrets Manager and KMS for every connector credential
