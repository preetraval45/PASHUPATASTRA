...
def main() -> int:
    ...
    with sync_playwright() as play:
        ...
        sampling = browser.new_context(viewport={"width": 1280, "height": 1000})
        page = sampling.new_page()
        ...
        for _ in range(SAMPLES):
            ...
            for row in rows:
                if row["hidden"] or row["empty"]:
                    failures.append(f"chain text hidden mid-sequence: {row['text']!r}")
            ...
            for marker in page.evaluate(
                ...
                if marker["h"] < 4 or marker["w"] < 4:
                    failures.append(
                        f"chain marker {marker['n']!r} collapsed to "
        ...
        if worst < MIN_OPACITY:
            failures.append(
                f"chain text dropped to opacity {worst:.2f} during the reveal "
                f"({worst_text!r}) — unreadable text is what this clause forbids"
            )
        else:
            print(f"across {SAMPLES} frames the faintest chain text was opacity {worst:.2f}")
