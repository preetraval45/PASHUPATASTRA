...
def main() -> int:
    ...
    definition = {
        ...
        "Target": {
            "Arn": target_arn,
            "RoleArn": role["Arn"],
            "Input": json.dumps({"task": "ingest_feeds"}),
            "RetryPolicy": {"MaximumRetryAttempts": 2},
        },
        "Description": "Hourly threat feed ingestion (CISA KEV, abuse.ch URLhaus).",
        "State": "ENABLED",
    }
...
def _explain(args, target_arn: str, role_arn: str, definition: dict) -> None:
    ...
    print(f"           schedule  rate({args.rate}), flexible window 15 min")
    print(f"           target    {target_arn}")
